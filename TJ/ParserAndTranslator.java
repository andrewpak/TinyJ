...
import static TJlexer.LexicalAnalyzer.getCurrentValue;
...
public final class ParserAndTranslator {
  ...
  private static void outputStmt() throws SourceFileErrorException
  {
    ...
    switch (getCurrentToken()) {
      ...
      case PRINT: accept(PRINT); accept(LPAREN); printArgument(); accept(RPAREN); accept(SEMICOLON); break;
      case PRINTLN: accept(PRINTLN); accept(LPAREN);
                    if(getCurrentToken() != RPAREN) {printArgument(); accept(RPAREN); accept(SEMICOLON);}
                    else {accept(RPAREN); accept(SEMICOLON);} new WRITELNOPinstr(); break;
  ...
  private static void expr1() throws SourceFileErrorException
  {
    ...
    switch (getCurrentToken()) {
      ...
      case PLUS: nextToken(); expr1(); break;
      case MINUS: nextToken(); expr1(); new CHANGESIGNinstr(); break;
      case NOT: nextToken(); expr1(); new NOTinstr(); break;
      case UNSIGNEDINT: new PUSHNUMinstr(getCurrentValue()); accept(UNSIGNEDINT); break;
      case NEW: nextToken(); accept(INT); accept(LBRACKET); expr3(); new HEAPALLOCinstr(); accept(RBRACKET);
